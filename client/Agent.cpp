...
Agent::Agent( int x, int y, int w, int h,SDL_Surface* agentTexture, float colisionFactor){
    width = w;
    height = h;
    ...
    colisionRect.x = x;
    colisionRect.y = y;
    colisionRect.w = w * colisionFactor;
    colisionRect.h = h * colisionFactor;
...
void Agent::renderAsItem(SDL_Surface* screenSurface, SDL_Point cameraPosition){
    destRect.x = mMovement.position.x - cameraPosition.x;
    destRect.y = mMovement.position.y - cameraPosition.y;
    SDL_BlitSurface(texture, &sourceRect, screenSurface, &destRect);
}

void Agent::setPosition(int x, int y, int mapSizeX, int mapSizeY){
    mMovement.position.x = x;
    mMovement.position.y = y;
    
    if (x > 512-width/2) {
        if (x > (64 * mapSizeX) - 512-width/2) {
            mMovement.cameraPosition.x = mapSizeX * 64 - (64 * (1024/64));
            mMovement.screenPosition.x =  x - (mapSizeX*64) + (1024);
        }else{
            mMovement.cameraPosition.x = mMovement.position.x - (512-width/2);
            mMovement.screenPosition.x = (512-width/2);
        }
    }else{
        mMovement.cameraPosition.x = 0;//posicao da camera no mapa
        mMovement.screenPosition.x = x;//posicao do boneco na tela
    }
    
    if (y > 320-height/2) {
        if (y > (64 * mapSizeY) - 320-height/2) {
            mMovement.cameraPosition.y = mapSizeY * 64 - (64 * (640/64));
            mMovement.screenPosition.y =  y - (mapSizeY*64) + (640);
        }else{
...
{
    if ((!mMovement.up && !mMovement.down) && (!mMovement.left && !mMovement.right)) {
